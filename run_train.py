...
from scripts.utils.train_utils import init_weights
...
if __name__ == "__main__":
    ...
    load_pretrained = args.pretrained if args.pretrained != "" else None

    scorer_path = str(Path(args.scorer_path).resolve().expanduser())
    key_answers_path = str(
        Path(args.scorer_result_dir).resolve().expanduser() / "answer_key.txt")
    results_path = str(
        Path(args.scorer_result_dir).resolve().expanduser() /
        "proposed_answer.txt")
    ...
    checkpoint_dir = Path(args.checkpoint_dir).resolve().expanduser()
    if resume:
        resume = str(checkpoint_dir / resume)

    pretrain = config["model_hyperp"]["modes"]["pretrain"] is True
    if load_pretrained is not None and not pretrain:
        load_pretrained = str(Path(args.pretrained).resolve().expanduser())
    else:
    ...
    train = Trainer(optim=optim,
                    ...
                    key_answers_path=key_answers_path,
                    **config["train_parms"])
    if resume is None:
        init_weights(model, load_pretrained)
    else:
        model_state = train.load_checkpoint()
